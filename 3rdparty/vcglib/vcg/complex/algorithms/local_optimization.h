...
// This function  is called from time to time by the doOptimization (e.g. when the heap is larger than fn*3)
  void ClearHeap()
  {
    ...
    //std::cout << "new heap size:" << h.size() << std::endl;
  }
  ...
  bool GoalReached() {
      /*if (IsTerminationFlag(LOnSimplices) && (m.SimplexNumber() <= nTargetSimplices)) {
          std::cout << "LOnSimplices" << std::endl; return true;
      }*/
      //if ( IsTerminationFlag(LOnVertices)  &&  ( m.VertexNumber() <= nTargetVertices)) return true;
      if (IsTerminationFlag(LOnOps) && (nPerformedOps == nTargetOps)) {
          std::cout << "LOnOps" << std::endl; return true;
      }
      if (IsTerminationFlag(LOMetric) && (currMetric > targetMetric)) {
          std::cout << "LOMetric" << std::endl; return true;
      }
