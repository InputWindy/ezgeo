...
public:
  ...
  double    CosineThr             = cos(M_PI / 2); // ~ cos(pi/2) 
  ...
  std::vector<unsigned int> PreservedFacesId;
...
public:
  ...
  typedef typename TriMeshType::EdgeType EdgeType;
  ...
  void ComputePosition(BaseParameterClass *_pp)
  {
    ...
    this->optimalPos = this->pos.V(1)->P();
  }
  ...
  // - sherlock lu
  static void MarkPreservedFaces(TriMeshType& M, const std::vector<unsigned int>& InFacesId)
  {
      for (size_t Idx = 0; Idx < InFacesId.size(); Idx++)
      {
          FaceType& Face = M.face[InFacesId[Idx]];

          // - Use 'USER0' bitmask to mark this vert as preserved .
          Face.V(0)->SetUserBit(VertexType::USER0);
          Face.V(1)->SetUserBit(VertexType::USER0);
          Face.V(2)->SetUserBit(VertexType::USER0);
      }
  }
  ...
  static void Init(TriMeshType& m, HeapType& h_ret, BaseParameterClass* _pp)
  {
      ...
      vcg::tri::UpdateTopology<TriMeshType>::VertexEdge(m);
      ...
      // - sherlock lu
      MarkPreservedFaces(m,pp->PreservedFacesId);
          ...
          for (auto vi = m.vert.begin(); vi != m.vert.end(); ++vi)
              if (!(*vi).IsD() && (*vi).IsRW())
              {
                  // - sherlock lu
                  {
                      // - mark vi as visited
                      (*vi).SetV();
                      if ((*vi).IsUserBit(VertexType::USER0)) { /*std::cout << "skip" << std::endl;*/ continue; }

                      vcg::face::VFIterator<FaceType> tf;
                      for (tf.F() = (*vi).VFp(), tf.I() = (*vi).VFi(); tf.F() != 0; ++tf)
                      {
                          if (tf.V0() < tf.V1() && tf.V1()->IsRW() && !tf.V1()->IsV())
                          {
                              h_ret.push_back(HeapElem(new MYTYPE(VertexPair(tf.V0(), tf.V1()), TriEdgeCollapse< TriMeshType, VertexPair, MYTYPE>::GlobalMark(), _pp)));
                          }

                      }
                      //// - iterate adjacent edges of this vertex
                      //vcg::edge::VEIterator<EdgeType> te;
                      //for (te.E() = (*vi).VEp(), te.I() = (*vi).VEi(); te.E() != 0; ++te)
                      //{
                      //    // - Use 'USER0' bitmask to mark this edge as preserved .
                      //    if (te.E()->IsUserBit(EdgeType::USER0)) { /*std::cout << "skip" << std::endl;*/ continue; }

                      //    // - push collapse vertex pair
                      //    if ((te.V0() < te.V1()) && te.V1()->IsRW() && !te.V1()->IsV())
                      //    {
                      //        // - What is Global mark ?
                      //        // - GlobalMark represents the number of iterations (heap update times)
                      //        // - initial 0
                      //        h_ret.push_back(HeapElem(new MYTYPE(VertexPair(te.V0(), te.V1()), TriEdgeCollapse< TriMeshType, VertexPair, MYTYPE>::GlobalMark(), _pp)));
                      //    }
                      //}
                  }
      ...
      {           
          ...
          //        {
          //            //sherlock lu
          //            // - filter preserved faces
          //            //if (IsPreservedFace(v_pf, x.F()->Index()))continue;
...
/** Evaluate the priority (error) for an edge collapse
  ...
  */
  ...
  inline  void UpdateHeap(HeapType& h_ret, BaseParameterClass* _pp)
  {
      /*
         - sherlock lu
         - GlobalIMark : Global Incremental mark (Update heap times)
         - v[1]->IMark() : Which update mark is this vertex in 
         - ClearV() : Clear visited flag
         - IsV() : Is visited
         - SetV() : Set visited flag
         - IsRW() : Is read write
      */

      this->GlobalMark()++;
      VertexType* v[2];
      v[0] = this->pos.V(0);// v0 is deleted and v1 take the new position
      v[1] = this->pos.V(1);
      ...
      for (VFIterator vfi(v[1]); !vfi.End(); ++vfi) 
      {
          bool b_valid1 = !(vfi.V1()->IsUserBit(VertexType::USER0)) && vfi.V1()->IsRW();
          bool b_valid2 = !(vfi.V2()->IsUserBit(VertexType::USER0)) && vfi.V2()->IsRW();

          if (b_valid1)
          {
              AddCollapseToHeap(h_ret, vfi.V1(), vfi.V0(), _pp);
          }
          ...
          if (b_valid1 && !b_valid2) AddCollapseToHeap(h_ret, vfi.V1(), vfi.V2(), _pp);
          if (!b_valid1 && b_valid2) AddCollapseToHeap(h_ret, vfi.V2(), vfi.V1(), _pp);
